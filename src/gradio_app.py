...
AVAILABLE_CATEGORIES = settings.arxiv.search_categories
...
async def stream_response(
    ...
    category_list = [cat.strip() for cat in categories.split(
        ",") if cat.strip()] if categories else None

    # Prepare request payload
    payload = {"query": query, "top_k": top_k, "use_hybrid": use_hybrid,
               "model": model, "categories": category_list}
                        ...
                        try:
                            ...
                            if "chunk" in data:
                                ...
                                if sources or chunks_used:
                                    ...
                                    formatted_response += f"- Mode: {
                                        search_mode}\n"
                                    formatted_response += f"- Chunks used: {
                                        chunks_used}\n"
                                    if sources:
                                        ...
                                        for i, source in enumerate(sources[:3], 1):
                                            formatted_response += f"  {
                                                i}. [{source.split('/')[-1]}]({source})\n"
                                        if len(sources) > 3:
                                            formatted_response += f"  ... and {
                                                len(sources) - 3} more\n"
            ...
            with gr.Column():
                with gr.Accordion("Advanced Options", open=False):
                    ...
                    model_choice = gr.Dropdown(
                        choices=["meta/llama-3.3-70b-instruct"],
                        value=DEFAULT_MODEL,
                        label="LLM Model",
        ...
        gr.Examples(
            examples=[
                ["What are transformers in machine learning?",
                    3, True, "meta/llama-3.3-70b-instruct", "cs.AI, cs.LG"],
                ["How do convolutional neural networks work?",
                    5, True, "meta/llama-3.3-70b-instruct", "cs.CV, cs.LG"],
                ["What is attention mechanism in deep learning?",
                    4, False, "meta/llama-3.3-70b-instruct", "cs.AI"],
                ["Explain reinforcement learning algorithms",
                    3, True, "meta/llama-3.3-70b-instruct", "cs.LG, cs.AI"],
                ["What are the latest developments in NLP?",
                    5, True, "meta/llama-3.3-70b-instruct", "cs.CL"],
            ],
