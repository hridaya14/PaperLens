...
class ArxivClient:
    ...
    async def fetch_papers(
        ...
        # Build search query
        if len(self.search_categories) == 1:
            search_query = f"cat:{self.search_categories[0]}"
        else:
            search_query = "(" + " OR ".join(
                [f"cat:{cat}" for cat in self.search_categories]) + ")"
        ...
        url = f"{self.base_url}?{
            urlencode(params, quote_via=quote, safe=safe)}"

        try:
            logger.info(f"Fetching {max_results} {
                        self.search_categories} papers from arXiv")

            # Rate limiting delay between all requests (arXiv recommends 3 seconds)
            ...
            raise ArxivAPIException(f"arXiv API returned error {
    ...
    async def fetch_paper_by_id(self, arxiv_id: str) -> Optional[ArxivPaper]:
        ...
        try:
            ...
            raise ArxivAPIException(f"arXiv API returned error {
                                    e.response.status_code} for paper {arxiv_id}: {e}")
        except Exception as e:
            logger.error(f"Failed to fetch paper {arxiv_id} from arXiv: {e}")
            raise ArxivAPIException(f"Unexpected error fetching paper {
                                    arxiv_id} from arXiv: {e}")
    ...
    def _parse_single_entry(self, entry: ET.Element) -> Optional[ArxivPaper]:
        ...
        try:
            ...
            abstract = self._get_text(
                entry, "atom:summary", clean_newlines=True)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"PDF download timeout (attempt {
                                   attempt + 1}/{max_retries}): {e}")
                    logger.info(f"Retrying in {wait_time}s...")
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(f"PDF download failed after {
                                 max_retries} attempts due to timeout: {e}")
                    raise PDFDownloadTimeoutError(f"PDF download timed out after {
                                                  max_retries} attempts: {e}")
            except httpx.HTTPError as e:
                if attempt < max_retries - 1:
                    wait_time = 5 * (attempt + 1)  # Exponential backoff
                    logger.warning(f"Download failed (attempt {
                                   attempt + 1}/{max_retries}): {e}")
                    logger.info(f"Retrying in {wait_time}s...")
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(f"Failed after {max_retries} attempts: {e}")
                    raise PDFDownloadException(f"PDF download failed after {
                                               max_retries} attempts: {e}")
            except Exception as e:
