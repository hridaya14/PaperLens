...
class DoclingParser:
    ...
    async def parse_pdf(self, pdf_path: Path) -> Optional[PdfContent]:
        ...
        try:
            ...
            result = self._converter.convert(str(
                pdf_path), max_num_pages=self.max_pages, max_file_size=self.max_file_size_bytes)
            ...
            if "too large" in error_msg or "too many pages" in error_msg:
                logger.info(
                    f"Skipping PDF processing due to size/page limits: {e}")
                return None
            else:
